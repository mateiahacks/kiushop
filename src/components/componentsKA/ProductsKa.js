                ...
                {loaded && <div onClick={() => {toggleShowAdd(); window.scrollTo(0, 0);}} className="addProduct">
                    ...
                    } */}
                    {
                        data.map((prod) => (prod.featured == true && type === "featured" || (prod.featured == false && type !== "featured") || type === "all") && (
                            <Product key={data.indexOf(prod)} lang="ka" id={prod.id} sale={prod.discount} name={prod.title_ge} price={prod.price} img={prod.images[prod.images.length-1] === undefined ? "":prod.images[prod.images.length-1].img_url}/>
                        ))
                    }
                </div>
            </div>
